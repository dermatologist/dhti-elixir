...
def test_chain_invoke(simple_chain, capsys):
    input_data = {"input": "Answer in one word: What is the capital of France?"}
    result = simple_chain.invoke(input=input_data)  # type: ignore
    print(result)
    captured = capsys.readouterr()
    assert "Paris" in captured.out or "know" in captured.out


def test_chain_invoke_with_hook(simple_chain, capsys):
    input_data = {
...
def test_chain_invoke_with_request(simple_chain, capsys):
    input_data = {"input":{
